...
function updateColors(textColHex) {
	...
	if (isLightBg) textColHex = lightenDarkenColor(textColHex, -15);
	// vibrant color is always too bright for white bg mode
...
async function songchange() {
	...
	if (!bgImage) {
		bgImage =
			"https://cdn.jsdelivr.net/gh/JulienMaille/spicetify-dynamic-theme@main/images/tracklist-row-song-fallback.svg";
		textColor = "#1db954";
		updateColors(textColor);
	}
	...
	if (album_uri && !album_uri.includes("spotify:show")) {
		...
		const episodeInfo = await getEpisodeInfo(
			Spicetify.Player.data.item.uri.replace("spotify:episode:", "")
		);
		let podcast_date = new Date(episodeInfo.release_date);
		podcast_date = podcast_date.toLocaleString("default", {
			year: "numeric",
			month: "short",
		});
	...
	if (!document.querySelector("#main-trackInfo-year")) {
		waitForElement(
			[".main-trackInfo-container:not(#upcomingSongDiv)"],
			(queries) => {
...
function pickCoverColor() {
	...
	if (Spicetify.Player.data.item.isLocal)
		img.src = Spicetify.Player.data.item.metadata.image_url;

	if (!img.complete) return setTimeout(pickCoverColor, 250); // Check if image is loaded

	textColor = "#1db954";
	if (
		Spicetify.Platform.PlatformData.client_version_triple >= "1.2.48" &&
		img.src.startsWith("https://i.scdn.co/image")
	) {
		var imgCORS = new Image();
		imgCORS.crossOrigin = "anonymous"; // Enable CORS
		imgCORS.src = Spicetify.Player.data.item.metadata.image_url.replace(
			"spotify:image:",
