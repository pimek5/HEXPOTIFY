...
(function initTrackFeatures() {
    ...
    let isEnabled = true;  // Enabled - these endpoints have better rate limits
    
    // Load settings from config
    function loadSettings() {
        try {
            const config = JSON.parse(localStorage.getItem('dynamicglow:config') || '{}');
            isEnabled = config.enableTrackFeatures === true;  // Explicit true required
            console.log('[DynamicGlow] Track Features', isEnabled ? 'enabled' : 'disabled');
        } catch (err) {
            console.log('[DynamicGlow] Using default Track Features settings (disabled)');
        }
    }
    ...
    async function getTrackFeatures(trackId) {
        if (!isEnabled) {
            console.log('[DynamicGlow] Track Features disabled - skipping API call');
            return null;
        }
    ...
    window.DynamicGlowTrackFeaturesAPI = {
        setEnabled: (enabled) => {
            isEnabled = enabled;
            console.log('[DynamicGlow] Track Features', enabled ? 'enabled' : 'disabled');
            if (!enabled) {
...
(function initThemeSettings() {
    const style = document.createElement('style');
    style.textContent = `
        .dynamicglow-theme-settings-button svg {
            width: 24px;
            height: 24px;
        }
    ...
    window.DynamicGlowThemeSettings = {
        createButton(referenceButton) {
            const button = document.createElement('button');
            button.className = referenceButton ? referenceButton.className : '';
            button.classList.add('dynamicglow-theme-settings-button');
            button.setAttribute('aria-label', 'DynamicGlow Theme Settings');
            button.setAttribute('title', 'DynamicGlow Theme Settings');
            button.innerHTML = `
...
// ===== REFRESH RING =====
(function initRefreshRing() {
    waitForElement(['button[aria-label="Reload page"]'], (queries) => {
        const reloadButton = queries[0];
        if (!reloadButton) return;

        const actionButtons = reloadButton.closest('.main-actionButtons');
        const parent = reloadButton.parentElement;
        if (!parent) return;

        function insertBeforeReload(node) {
            const wrapper = reloadButton.closest('div');
            if (actionButtons && wrapper && wrapper.parentElement === actionButtons) {
                actionButtons.insertBefore(node, wrapper);
            } else {
                reloadButton.insertAdjacentElement('beforebegin', node);
            }
        }

        function createSettingsButtonFrom(target) {
            const customButton = document.createElement('button');
            customButton.className = target.className + ' dynamicglow-topbar-button';
            customButton.setAttribute('aria-label', 'DynamicGlow Settings');
            customButton.setAttribute('title', 'DynamicGlow Settings');

            const svg = target.querySelector('svg');
            if (svg) {
                customButton.appendChild(svg.cloneNode(true));
            }

            customButton.addEventListener('click', () => {
                if (window.DynamicGlowConfig?.open) {
                    window.DynamicGlowConfig.open();
                } else {
                    Spicetify.showNotification('DynamicGlow: config not available.');
                }
            });
            return customButton;
        }
        ...
        function ensureButtons() {
            if (!parent.querySelector('.dynamicglow-theme-settings-button')) {
                const themeButton = window.DynamicGlowThemeSettings?.createButton
                    ? window.DynamicGlowThemeSettings.createButton(reloadButton)
                    : null;
                if (themeButton) {
                    insertBeforeReload(themeButton);
                }
            }

            if (!parent.querySelector('.dynamicglow-topbar-button')) {
                const settingsButton = createSettingsButtonFrom(reloadButton);
                insertBeforeReload(settingsButton);
            }
        }
        ...
        if (actionButtons) {
            ...
            observer.observe(actionButtons, { childList: true, subtree: true });
        }

        if (!parent.querySelector('.dynamicglow-refresh-ring')) {
            const ring = document.createElement('span');
            ring.className = 'dynamicglow-refresh-ring';
            reloadButton.insertAdjacentElement('afterend', ring);
        }
    });
})();

// ===== SIDEBAR COVER IMAGES =====
(function initSidebarCoverImages() {
    let isEnabled = true;  // Enabled - these endpoints have better rate limits
    ...
    function loadSettings() {
        try {
            const config = JSON.parse(localStorage.getItem('dynamicglow:config') || '{}');
            isEnabled = config.enableSidebarCovers === true;  // Explicit true required
            console.log('[DynamicGlow] Sidebar Covers', isEnabled ? 'enabled' : 'disabled');
        } catch (err) {
            console.log('[DynamicGlow] Using default Sidebar Covers settings (disabled)');
        }
    }
    
    loadSettings();
    
    const ROW_SELECTOR = '.main-yourLibraryX-listItem';
    const ROW_CLASS = 'dynamicglow-sidebar-row';
    const COVER_CLASS = 'dynamicglow-sidebar-cover';
    const REQUEST_DELAY_MS = 2000;  // 2 seconds delay to reduce API load
    ...
    function parseHref(href) {
        if (!href) return null;
        const match = href.match(/^\/(playlist|album|artist|show)\/([A-Za-z0-9]+)$/);
        if (!match) return null;
        return { type: match[1], id: match[2] };
    }

    function getExistingImage(row) {
    ...
    function fetchCover(type, id) {
        if (!isEnabled) {
            console.log('[DynamicGlow] Sidebar Covers disabled - skipping API call');
            return Promise.resolve(null);
        }
        
        const key = `${type}:${id}`;
        if (coverCache.has(key)) {
            return Promise.resolve(coverCache.get(key));
        }
        if (inflight.has(key)) {
        ...
        const promise = new Promise((resolve) => {
            queue.push(async () => {
                try {
                    let endpoint = null;
                    if (type === 'playlist') {
                        endpoint = `https://api.spotify.com/v1/playlists/${id}?fields=images`;
                    } else if (type === 'album') {
                        endpoint = `https://api.spotify.com/v1/albums/${id}?fields=images`;
                    } else if (type === 'artist') {
                        endpoint = `https://api.spotify.com/v1/artists/${id}?fields=images`;
                    } else if (type === 'show') {
                        endpoint = `https://api.spotify.com/v1/shows/${id}?fields=images`;
                    }
                    ...
                    console.log('[DynamicGlow] Sidebar cover fetch failed', err);
                    coverCache.set(key, null);
                    resolve(null);
                } finally {
                    inflight.delete(key);
    ...
    async function updateRow(item) {
        const row = item.querySelector('button, a');
        if (!row) return;

        row.classList.add(ROW_CLASS);

        let cover = row.querySelector(`.${COVER_CLASS}`);
        if (!cover) {
        ...
        const existing = getExistingImage(row);
        if (existing?.src) {
            cover.style.backgroundImage = `url("${existing.src}")`;
            return;
        }

        const link = row.getAttribute('href') || row.closest('a')?.getAttribute('href');
        const parsed = parseHref(link);
        if (!parsed) return;

        const url = await fetchCover(parsed.type, parsed.id);
        if (url) {
    ...
    function scanRows() {
        ...
        document.querySelectorAll(ROW_SELECTOR).forEach((item) => {
            updateRow(item);
        });
    }
    
    // Expose API for config menu
    window.DynamicGlowSidebarAPI = {
        setEnabled: (enabled) => {
            isEnabled = enabled;
            console.log('[DynamicGlow] Sidebar Covers', enabled ? 'enabled' : 'disabled');
            if (enabled) {
                scheduleScan();
            }
        },
